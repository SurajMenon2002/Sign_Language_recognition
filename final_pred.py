...
class Application:
    ...
    def __init__(self):
        ...
        self.root = tk.Tk()
        self.root.title("Sign Language Interpretation")
        self.root.protocol('WM_DELETE_WINDOW', self.destructor)
        self.root.geometry("1300x700")
        self.root.config(bg="white")

        self.panel = tk.Label(self.root)
        self.panel.place(x=100, y=70, width=480, height=480)
        self.panel.config(bg="white")

        self.panel2 = tk.Label(self.root)  # initialize image panel
        self.panel2.place(x=620, y=70, width=400, height=400)
        self.panel2.config(bg="white")
        
        sign_bgr = cv2.imread("C:\\Users\\dipti\\Downloads\\Sign-Language-To-Text-and-Speech-Conversion-master\\Sign-Language-To-Text-and-Speech-Conversion-master\\ASL.jpg")
        sign_rgb = cv2.cvtColor(sign_bgr, cv2.COLOR_BGR2RGB)
        sign_image = Image.fromarray(sign_rgb)
        display = ImageTk.PhotoImage(image=sign_image)
        self.panel9 = tk.Label(self.root, image=display)
        self.panel9.image = display
        self.panel9.place(x=1050, y=70, width=400, height=400)

        self.T = tk.Label(self.root)
        self.T.place(x=60, y=5)
        self.T.config(text="Sign Language Interpretation", bg="white", font=("Tahoma", 20))
        ...
        self.T1.config(text="Character :", font=("Tahoma", 20, "bold"),bg="white")
        ...
        self.T3.config(text="Sentence :", font=("Tahoma", 20, "bold"), bg="white")

        self.T4 = tk.Label(self.root)
        self.T4.place(x=10, y=700)
        self.T4.config(text="Suggestions :", fg="green", font=("Tahoma", 20, "bold"), bg="white")


        self.b1=tk.Button(self.root)
        self.b1.place(x=390,y=700)
        self.b1.config(bg="white", fg="black", bd=0, font=("Tahoma", 10, "bold"), highlightthickness=0, relief="flat")

        self.b2 = tk.Button(self.root)
        self.b2.place(x=590, y=700)
        self.b2.config(bg="white", fg="black", bd=0, font=("Tahoma", 10, "bold"), highlightthickness=0, relief="flat")

        self.b3 = tk.Button(self.root)
        self.b3.place(x=790, y=700)
        self.b3.config(bg="white", fg="black", bd=0, font=("Tahoma", 10, "bold"), highlightthickness=0, relief="flat")

        self.b4 = tk.Button(self.root)
        self.b4.place(x=990, y=700)
        self.b4.config(bg="white", fg="black", bd=0, font=("Tahoma", 10, "bold"), highlightthickness=0, relief="flat")

        self.speak = tk.Button(self.root)
        self.speak.place(x=1305, y=700)
        self.speak.config(text="Speak", font=("Tahoma", 20), wraplength=100, bg="#4CAF50", fg="white", activebackground="#66BB6A", activeforeground="white", borderwidth=0, highlightthickness=0, relief="flat", command=self.speak_fun)

        self.clear = tk.Button(self.root)
        self.clear.place(x=1205, y=700)
        self.clear.config(text="Clear", font=("Tahoma", 20), wraplength=100, bg="#4CAF50", fg="white", activebackground="#66BB6A", activeforeground="white", borderwidth=0, highlightthickness=0, relief="flat", command=self.clear_fun)
    ...
    def video_loop(self):
        try:
            ...
            hands_data = hd.findHands(cv2image, draw=False, flipType=True)
            ...
            if hands_data:   # Check if hands_data contains information
                first_hand_data = hands_data[0][0]  # Access the first hand's data
                if 'bbox' in first_hand_data:
                    x, y, w, h = first_hand_data['bbox']
                    image = cv2image_copy[y - offset:y + h + offset, x - offset:x + w + offset]

                    white = cv2.imread("C:\\Users\\dipti\\Downloads\\Sign-Language-To-Text-and-Speech-Conversion-master\\white.jpg")

                    handz_data = hd2.findHands(image, draw=False, flipType=True)
                    if handz_data and handz_data[0]:
                        first_handz_data = handz_data[0][0]
                        if 'lmList' in first_handz_data:
                            self.pts = first_handz_data['lmList']
                            os = ((400 - w) // 2) - 15
                            os1 = ((400 - h) // 2) - 15
                            ...
                            # sign = cv2.imread("C:\\Users\\dipti\\Downloads\\Sign-Language-To-Text-and-Speech-Conversion-master\\ASL.jpg")  # Adjust the path to your image
                            # display = ImageTk.PhotoImage(image=self.sign)
                            # self.panel9.display = display
                            # self.panel9.place(x=1100, y=115, width=400, height=400)
                            # self.panel9.config(image=self.image)
                            ...
                            self.panel3.config(text=self.current_symbol, font=("Tahoma", 20))

                            #self.panel4.config(text=self.word, font=("Tahoma", 20))



                            self.b1.config(text=self.word1, font=("Tahoma", 16), wraplength=825, command=self.action1)
                            self.b2.config(text=self.word2, font=("Tahoma", 16), wraplength=825,  command=self.action2)
                            self.b3.config(text=self.word3, font=("Tahoma", 16), wraplength=825,  command=self.action3)
                            self.b4.config(text=self.word4, font=("Tahoma", 16), wraplength=825,  command=self.action4)

            self.panel5.config(text=self.str, font=("Tahoma", 20), wraplength=1025)
        except Exception:
